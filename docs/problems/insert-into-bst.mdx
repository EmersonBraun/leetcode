...
console.log('Example 2 - Right left value:', result2.right.left.val); // 5`}
  testCases={[
    { input: '(() => { const r = new TreeNode(2); r.left = new TreeNode(1); r.right = new TreeNode(3); return insertIntoBST(r, 4).right.right.val; })()', expected: '4' },
    { input: '(() => { const r = new TreeNode(4); r.left = new TreeNode(2); r.right = new TreeNode(7); r.left.left = new TreeNode(1); r.left.right = new TreeNode(3); return insertIntoBST(r, 5).right.left.val; })()', expected: '5' }
  ]}
/>
